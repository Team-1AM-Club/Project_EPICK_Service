...
from app.services.core_decision_inbound import (
    CoreDecisionInboundError,
    CoreDecisionInboundService,
)
...
class AuthenticatedDeliveryContext:
    """Identity derived from SQS system metadata, never from the event body."""
...
@dataclass(frozen=True, slots=True)
class W3CoreDecisionWorkerResult:
    received: int
    acknowledged: int
    retry_scheduled: int
    applied: int
...
class W3CoreDecisionWorker:
    """Apply authenticated W3 decisions and ACK only after a durable DB outcome."""
    ...
    def __init__(
        ...
        *,
        ...
        expected_producer: str = "w3",
        batch_size: int = _MAX_SQS_BATCH_SIZE,
        visibility_timeout_seconds: int = 120,
        wait_time_seconds: int = 20,
        service_factory: Callable[[Any], CoreDecisionInboundService] = CoreDecisionInboundService,
    ) -> None:
        ...
        if expected_producer != "w3":
            raise ValueError("expected_producer must be w3")
    ...
    def drain_once(self) -> W3CoreDecisionWorkerResult:
        try:
            ...
            return W3CoreDecisionWorkerResult(0, 0, 1, 0, 0, 0)
        ...
        for delivery in deliveries:
            ...
            if context is None:
                ...
                continue
            try:
                event = parse_w3_core_decision_event(delivery.body)
                if event.producer != self._expected_producer:
                    raise CoreDecisionContractError("CORE_DECISION_PRODUCER_MISMATCH")
                with self._session_factory.begin() as session:
                    receipt = self._service_factory(session).apply(
                        body=delivery.body,
                        authenticated_principal=context.logical_principal,
                    )
            except (CoreDecisionContractError, CoreDecisionInboundError):
                terminal_rejected += 1
                if self._delete(delivery.receipt_handle):
                    acknowledged += 1
                else:
            ...
            if (
                receipt.retryable
                or receipt.outcome is CoreDecisionReceiptOutcome.RETRYABLE_INFRA_FAILURE
            ):
                retry_scheduled += 1
                continue
            if receipt.outcome is CoreDecisionReceiptOutcome.APPLIED:
                applied += 1
            elif receipt.outcome is CoreDecisionReceiptOutcome.DUPLICATE:
                duplicate += 1
            else:
                terminal_rejected += 1
            if self._delete(delivery.receipt_handle):
                ...
                retry_scheduled += 1
        return W3CoreDecisionWorkerResult(
    ...
    def _authenticate(self, *, sender_id: str | None) -> AuthenticatedDeliveryContext | None:
        ...
        return AuthenticatedDeliveryContext(
            sender_id=sender_id,
            logical_principal=self._expected_producer,
        )
