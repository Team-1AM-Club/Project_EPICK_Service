...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'epick_w4_context') THEN
        CREATE ROLE epick_w4_context NOLOGIN NOSUPERUSER NOBYPASSRLS;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'epick_w3_authority') THEN
        CREATE ROLE epick_w3_authority NOLOGIN NOSUPERUSER NOBYPASSRLS;
    END IF;
...
GRANT USAGE ON SCHEMA public
    TO epick_runtime, epick_worker, epick_lookup, epick_w3_authority, epick_w4_context, epick_deleter;
